...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merge Metis low-rank checkpoint tensors into standard linear weights.")
    parser.add_argument(
        "input_file",
        nargs="?",
        default="outputs/grpo/Qwen2_5-0.5B-instruct-fp4/checkpoint-300/model.safetensors",
        help="Input safetensors checkpoint.",
    )
    parser.add_argument(
        "--output-file",
        default=None,
        help="Output safetensors path. Defaults to '<input>_merged.safetensors'.",
    )
    args = parser.parse_args()

    input_file = args.input_file
    output_file = args.output_file or input_file.replace(".safetensors", "_merged.safetensors")
